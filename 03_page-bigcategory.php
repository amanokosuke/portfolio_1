<!--
Template Name: サービスカテゴリー
Template Post Type: list
-->
<?php
$parent = get_the_ID();
$category_name = get_post_meta($parent, "category_name", true);
$category_content = get_post_meta($parent, "category_content", true);
		...
		<div class="index_contents">
			<div class="pankuzu">
				<div class="breadcrumbs">
					<a href="/">トップページ</a> ＞ <?= $category_name ?>
				</div>
			</div>


			<div class="category_service category_list_content">
				<div class="subcategory_list">
                    <?php
                    $children1 = get_service_categories($term->term_id);
                    foreach ($children1 as $child1) {
                        ?>
						<div>
							<a href="/list/<?= $term->slug ?>/<?= $child1->slug ?>/"><?= $child1->name ?></a>
						</div>
                        <?php
                    }
                    ?>
...
<!-- 							</a> -->
 				...
 				<h2><?= $category_name ?>とは</h2>
				<p><?= nl2br(post_custom('category_content')); ?></p> 

				<h2><?= $category_name ?>に関する記事</h2>
                        ...
                        foreach ($posts as $post):
							...
							<article class="article_flame">
								...
								<div class="article_flame_block flex_start">
									<div class="article_flame_date">
                                        <?php echo get_the_modified_time('Y/m/d', $post->ID) ?></div>
									<div class="article_flame_author"><?php echo get_post_meta($post->ID, 'article_author', true); ?></div>
<!-- 									<div class="article_flame_tag"><?php
                                        if ($terms = get_the_terms($post->ID, 'service_tag')) {
                                            foreach ($terms as $term) {
                                                $term_slug = $term->slug;
                                                echo "<div><a>" . $term->name . '</a></div>';
...
<!-- 								<div class="article_flame_block">
									<div class="article_flame_category">
                                        <?php
                                        $cate = get_post_meta($post->ID, 'article_category', true);
                                        $subcate = get_post_meta($post->ID, 'article_subcategory', true);
                                        ?>
										<div><?= $cate ?><?php if ($subcate != null && strlen($subcate) > 0) {
                                                echo '>' . $subcate; } ?></div>
									</div>
